...
def calc_snr(unsuppressed_one,unsuppressed_two,roi_proportion=0.8):
    ...
    # Threshold regions
    otsu_thresh = threshold_otsu(np_unsuppressed_one)
    np_mask = np_unsuppressed_one > otsu_thresh
    np_mask = np_mask.astype(int) #Recode booleans as integers
    ...
    left_std_dev = np_difference_image_left.std()
    left_mean = np_difference_image_left.mean()

    right_std_dev = np_difference_image_right.std()
    right_mean = np_difference_image_right.mean()
