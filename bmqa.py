...
snr_results = measure.snr(get_mid_slice(images['snr_acquisition_one'].pixel_array), get_mid_slice(images['snr_acquisition_two'].pixel_array))
study['SNR_left'] = snr_results['left_snr']
study['SNR_right'] = snr_results['right_snr']
spir_results = measure.fse(get_mid_slice(images['spir_fat'].pixel_array), get_mid_slice(images['spir_water'].pixel_array))
study['SPIR-FSE_left'] = spir_results['left_fse']
study['SPIR-FSE_right'] = spir_results['right_fse']
spair_results = measure.fse(get_mid_slice(images['spair_fat'].pixel_array), get_mid_slice(images['spair_water'].pixel_array))
...
with open(csv_rel_path, 'w', newline='') as f:
    ...
    print('results saved in ' + str(csv_rel_path))
